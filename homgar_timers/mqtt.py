...
from .const import ZONE_RUNNING_FLAG, ZONE_FLAG_BYTE_OFFSET, STOP_TS_BYTE_OFFSET, DURATION_BYTE_OFFSET
...
class HomGarMQTTClient:
    def __init__(self, iot_credentials: dict, on_state_update: Callable[[str, dict], None]):
    ...
    def _on_disconnect(self, client, userdata, rc):
        self._connected = False
        _LOGGER.warning("HomGar MQTT disconnected rc=%s", rc)
    ...
    def send_open(self, hub_mid, timer_addr: int, zone_addr: int, duration_seconds: int) -> bool:
        if not self._connected or not self._client:
            _LOGGER.error("HomGar MQTT not connected")
            return False
        d_key = f"D{str(timer_addr).zfill(2)}"
        return self._publish(hub_mid, d_key, build_open_command(
            self.get_current_payload(hub_mid, d_key), zone_addr, duration_seconds))

    def send_close(self, hub_mid, timer_addr: int) -> bool:
        if not self._connected or not self._client:
            _LOGGER.error("HomGar MQTT not connected")
            return False
        d_key = f"D{str(timer_addr).zfill(2)}"
        return self._publish(hub_mid, d_key, build_close_command(
            self.get_current_payload(hub_mid, d_key)))

    def _publish(self, hub_mid, d_key: str, payload_hex: str) -> bool:
        try:
            ts = int(time.time() * 1000)
            param_header = f"P{time.strftime('%y%m%d%H%M%S')}0000028174{str(hub_mid).zfill(8)}"
            d_updates = {d_key: {"time": ts, "value": payload_hex}, "update": {"time": ts, "value": 1}}
            param_str = f"#{param_header}|{json.dumps(d_updates)}|{ts}|#"
            msg = json.dumps({"method": "thing.service.property.set",
                "id": str(ts % 2_147_483_647),
                "params": {"param": param_str}, "version": "1.0.0"})
            result = self._client.publish(self._topic, msg, qos=1)
            _LOGGER.info("Published hub=%s %s rc=%s", hub_mid, d_key, result.rc)
            return result.rc == 0
        except Exception as e:
            _LOGGER.error("HomGar MQTT publish failed: %s", e)
            return False
